...
public class SendAnswerActivity extends BaseActivityWithToolBar {

    String tag = "SendAnswerActivity";
    Context context;
    ...
    void send() {
        ...
        if (TextUtils.isEmpty(content)) {
            ToastUtil.show(context, "回答内容不能为空");
            return;
        }
        new InitiationService().answer(content, AppConfig.getUid(preference), questionId, new DefaultResponseListener<BaseResponse>() {
            @Override
